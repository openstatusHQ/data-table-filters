...
col.timestamp().minSize(200); // Floor for a flexing (unsized) column
```

A column without `.size()` flexes to absorb the table's leftover width. Give it a `.minSize()` floor so it never gets compressed below a usable width.

### Sorting & Optionality

```ts
col.number().sortable(); // Click-to-sort on column header
col.presets.timestamp().sortable(false); // Switch it off again on a preset
