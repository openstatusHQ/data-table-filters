...
faq:
  ...
  - question: "How do I paginate results?"
    answer: "Pagination is cursor-based, using the same fields as your table's UI. Pass cursor (Unix ms timestamp), size (rows per page, default 40), and direction ('next' or 'prev') inside the filters object. To get the next page, use the date of the last row as the cursor. Call with format 'stats' first to get the total count without fetching rows."
  - question: "What are common use cases?"
    answer: "Conversational data exploration (ask Claude Desktop or Cursor questions in natural language), monitoring and alerting (agents poll stats for anomalies), post-deploy validation (CI/CD agents check for error spikes), cross-table orchestration (agents query multiple endpoints to answer cross-cutting questions), internal tooling for non-developers (chat-based access to production data), and agent-to-agent pipelines (one agent's output feeds another's input)."
---
...
> Looking for the other direction? These docs are themselves an MCP server, so
> an agent can ask how to build the table before it queries one — see
> [For AI Agents](/docs/agents).

## Installation

```bash
npx shadcn@latest add https://data-table.openstatus.dev/r/data-table-mcp.json
```
...
Create a route handler co-located with your table's API. Export POST, GET, and
DELETE for Streamable HTTP spec compliance. POST carries every request; GET is
answered with 405, since the handler is stateless and has nothing to push on the
