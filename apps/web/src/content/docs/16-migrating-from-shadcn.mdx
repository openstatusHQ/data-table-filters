...
// Auto-generate columns and faceted filter definitions
...
import { columns, filterFields, paymentSchema } from "./table-schema";
import type { Payment } from "./types";

export function PaymentsTable({ data }: { data: Payment[] }) {
  const adapter = useNuqsAdapter(paymentSchema.definition);
...
2. **Add a ⌘K Command Palette:**
   ...
   <DataTableInfinite
     ...
     data={data}
     filterFields={filterFields}
     commandSlot={<DataTableFilterCommand schema={paymentSchema.definition} />}
   />;
   ```
...
import { paymentSchema } from "@/app/payments/table-schema";

const handler = createDrizzleHandler({
  db,
  table: payments,
  schema: paymentSchema.definition,
  columnMapping: {
    id: payments.id,
