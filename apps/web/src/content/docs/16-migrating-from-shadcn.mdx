...
> **Prerequisite.** Works on either shadcn library: the CLI default, Base UI (`npx shadcn@latest init -d`), or Radix (`npx shadcn@latest init -b radix -p lyra`). The CLI resolves primitives from whichever base your `components.json` specifies.
...
2. **Add a ⌘K Command Palette:**
   ...
   ```bash
   npx shadcn@latest add @data-table-filters/data-table-filter-command
   ```
...
```bash
npx shadcn@latest add @data-table-filters/data-table-drizzle
```
