...
The page param React Query carries is `{ page, _meta }` — the strategy owns
`page`, and the [meta-skipping](#skipping-metadata-on-pagination) flag sits
outside it so the same flag works whether pages are addressed by cursor, opaque
token, or offset. `getMetaPage` reads `_meta` and is unaffected by which
strategy is in use.
