...
The descriptor carries a `version` field. `createTableSchema.fromJSON` runs `migrateSchemaJSON` first, so JSON written by an older build still loads.

> Custom renderers (`display("custom", ...)`, `filter.component`, `sheet.component`, `sheet.condition`) are functions and cannot be serialized. They are stripped during `toJSON()` and must be applied manually on reconstructed schemas. The column's _other_ state is not affected: a column with a custom cell still serializes its real display type, so the reconstructed schema renders the built-in cell for that kind rather than falling back to plain text.
