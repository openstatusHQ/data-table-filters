...
The registry is `@data-table-filters` in the shadcn directory, so an agent with
a shell can list and inspect the blocks with the CLI alone, no docs fetch and no
`components.json` entry needed:

```bash
npx shadcn@latest search @data-table-filters
npx shadcn@latest view @data-table-filters/data-table
```
