...
`confirm` on a descriptor opens an alert dialog with `{count}` interpolated;
...
`filterRowCount` — the rows matching the filters alone. The server measures
drift on that same set, then applies within the action's `when` guard, so
"applied to 12 of 40 matching" is the guard at work, not a partial failure.
...
`delete`. Because it runs against a shared database, writes are opt-in: set
`ALLOW_DEMO_ACTIONS=1` and the list endpoint starts advertising actions;
without it, `POST /drizzle/api/actions/*` answers 403 and the UI renders no
buttons.
