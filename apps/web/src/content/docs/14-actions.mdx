---
title: "Row Actions"
description: "Declare actions next to their Drizzle handlers; the table renders row menus and a bulk bar from server metadata"
author: "openstatus"
...
faq:
  ...
  - question: "Can an action apply to every row matching the filters?"
    answer: "On the wire, yes: scope 'filter' sends the filter values plus expected_count, and inside the transaction the handler counts the set first under REPEATABLE READ and answers 409 count_mismatch with the actual number if it drifted. The block ships no UI for it — build your own trigger on useDataTableActions if you want one."
---
...
```bash
npx shadcn@latest add @data-table-filters/data-table-actions
```
...
Descriptors are server-authored and choose the URL the provider POSTs row ids
to, so they are validated once on arrival and anything that fails is dropped
rather than rendered — a button that cannot be sent safely is worse than no
button. Rejections are reported to the console, not the UI.

An `href` is accepted when it is a root-relative path (`/api/actions/replay`),
or an absolute `http(s)` URL on the page's own origin. Everything else is
rejected: protocol-relative URLs that read like a path
(`//evil.example.com/steal`), `javascript:`, `data:`, `blob:`,
directory-relative paths, control characters, and anything over 2048 chars.
The rest of the descriptor is bounded too — `id`, `label`, `scope`, `variant`,
`confirm` and `maxIds` are all range- and type-checked, and a duplicate `id` is
dropped.

When the actions endpoint lives on another origin, name it:
...
This matters most when the table renders a schema from a server you do not
control — see [Headless Tables](/docs/headless#trust-boundary).
