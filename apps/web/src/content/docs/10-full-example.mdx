...
> The `commandSlot` above uses the AI-enhanced command palette. It handles both structured queries (`host:api`) and natural language (_"slow requests from eu"_) in the same input. See [AI Filters](/docs/ai-filters) for the full setup including the API route.

## Live Examples

| Route                  | Pagination      | Data Source | Live Mode | Chart |
| ---------------------- | --------------- | ----------- | --------- | ----- |
| [/auto](/auto)         | Infinite scroll | In-memory   | No        | No    |
