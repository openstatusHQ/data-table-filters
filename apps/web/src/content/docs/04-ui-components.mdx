...
faq:
  ...
  - question: "What cell renderers are available?"
    answer: "12 cell renderers (text, code, number, bar, heatmap, gauge, timestamp, badge, boolean, star, status-code, level-indicator), used automatically by generateColumns and by the row detail sheet."
---
...
Registration is only needed for columns you write by hand that refer to a filter function by
name. In TanStack Table v9 that registration lives on the feature set, not on the table config:
...
export const dataTableFeatures = tableFeatures({
  ...
  filterFns: {
    // The set `filterFn: 'auto'` resolves to — register all of it, see below.
    ...
    // Ours. `inDateRange` deliberately shadows the v9 built-in of the same name.
    inDateRange,
    arrSome,
  },
});
...
`filterFns` slot on `tableFeatures()` instead (see [Filter Functions](#filter-functions)).
