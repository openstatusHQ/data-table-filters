...
Rows are ordered by `cursorColumn`, then by the user's sort, then by a unique tiebreak column — without that last one, rows sharing a timestamp come back in whatever order the query plan produced, and that order changes as rows are written. The tiebreak defaults to the table's single-column primary key. A table with a composite primary key (or none) has to name one itself:

```ts
const handler = createDrizzleHandler({
  // ...
  cursorColumn: "date",
  tiebreakColumn: "uuid", // must be unique
});
```
...
function ClientInner() {
  ...
  return (
    <DataTableInfinite
      columns={columns}
      data={flatData}
      totalRows={metaPage?.meta?.totalRowCount}
      filterRows={metaPage?.meta?.filterRowCount}
...
- **`buildCursorPagination({ cursor, direction, size, cursorColumn, tiebreakColumn? })`** — returns cursor condition, order clause, an optional tiebreak clause, and a `needsReverse` flag for bidirectional infinite scroll.
