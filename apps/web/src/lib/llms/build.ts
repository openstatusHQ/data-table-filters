...
export function buildLlmsTxt(sections: SectionMeta[]): string {
  ...
  lines.push(
    ...
    "## Optional",
    "",
    `- [MCP server](${BASE_URL}/api/mcp): these docs as tools — \`search_docs\`, \`get_doc\`, \`list_blocks\`, \`get_install_plan\`. Streamable HTTP, no auth`,
    ...
    `- [Agent skill](https://github.com/openstatushq/data-table-filters): \`${SKILL_INSTALL_COMMAND}\``,
    "",
  );
...
export function buildRegistryIndexMd(): string {
  ...
  lines.push(
    ...
    "## More",
    "",
    `- These docs as an MCP server: ${BASE_URL}/api/mcp`,
