...
The timeline chart helps you to visualize the data over time. It utilizes the shadcn component (checkout out the [docs](https://ui.shadcn.com/docs/components/chart)) and renders the bars based on the the amount of data items you defined in the `{ timestamp: number; [key: string]: number }[];` array.

As default, the key is the `level` that takes "success", "warning", "error" as values.
