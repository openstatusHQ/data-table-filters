...
There are two approaches to defining filters:

### Traditional Approach (nuqs-based)

For URL-based state management (like `/infinite` and `/light` routes), define filters in two places:

- `filterFields` in the `constants.tsx` file
- `searchParams` in the `search-params.ts` file

The `filterFields` is the source of truth for the UI. The `searchParams` are used to filter the data on the server side and to build the search command. Both must be kept in sync.

### BYOS Approach (Schema-based)

For client-side state management (like `/default` route), use the [BYOS schema](#byos-bring-your-own-store) to define filters in a single place. The schema acts as the single source of truth for types, serialization, and the filter command.

> **Tip**: If you're building a new data-table, consider using BYOS for simpler filter management with better type safety.
...
> **Alternative**: With BYOS, you can use a single schema definition instead of maintaining both `filterFields` and `searchParams`. See [BYOS](#byos-bring-your-own-store) for details.
...
### Server Prefetch & Loading States

The `/infinite` route supports server-side prefetching with React Query's `HydrationBoundary`. This can be toggled via a cookie-based flag, allowing you to compare the UX between server prefetch (with skeleton) and client-only fetching (with loading spinner).

#### Prefetch Toggle

The prefetch behavior is controlled by the `data-table-prefetch` cookie. When enabled, the server prefetches the initial data and streams it to the client with a `Skeleton` fallback during the loading state.
...
The `DataTableInfinite` component accepts a `showPrefetchToggle` prop to render a toggle button in the toolbar:

```tsx
<DataTableInfinite
  // ... other props
  showPrefetchToggle // Shows the prefetch toggle in toolbar
/>
```

#### Skeleton Component

The `Skeleton` component (`skeleton.tsx`) provides a loading state that matches the layout of the data-table. It includes:

- Left sidebar filter controls skeleton
- Command palette skeleton
- Toolbar skeleton
- Timeline chart skeleton with animated bars
- Table header and rows skeleton

This approach replaces the traditional `loading.tsx` file, giving you conditional control over when to show the skeleton based on the prefetch state.

#### When to Use Each Approach

| Approach              | Use Case                                                      |
| --------------------- | ------------------------------------------------------------- |
| **Prefetch enabled**  | Fast API responses, SEO-critical pages, avoid layout shift    |
| **Prefetch disabled** | Slow API responses, better perceived performance with spinner |

> **Note**: The prefetch toggle is only available on the `/infinite` route for demonstration purposes. The `/light` route never prefetches.

## BYOS (Bring Your Own Store)

BYOS is an adapter pattern that allows you to use any state management solution for your data-table filters. Instead of being locked into `nuqs` for URL-based state, you can use Zustand, or even create your own custom adapter.

### Why BYOS?

- **SPAs without URL routing**: Not every app needs filter state in the URL
- **Client-side only state**: Embedded widgets, modals, or dashboards where URL state is undesirable
- **Flexibility**: Use the state management solution you're already using in your app
...
export const filterSchema = createSchema({
  ...
  regions: field
    .array(field.stringLiteral(["ams", "gru", "syd", "iad"]))
    .delimiter(","),
  tags: field.array(field.stringLiteral(["api", "web", "app"])).delimiter(","),
...
field
  ...
  .delimiter(","); // Set serialization delimiter
```

### Using with DataTableFilterCommand

Pass the schema definition to the `DataTableFilterCommand` component:
...
The command will use the schema to parse and serialize filter values, enabling the text-based filter input to work with your custom schema.

### Creating a Custom Adapter

To create a custom adapter, implement the `DataTableAdapter` interface:

```ts
interface DataTableAdapter<TState extends Record<string, unknown>> {
  // Unique identifier for namespacing
  readonly id: string;
...
```tsx
import { getSchemaDefaults, Schema } from "@/lib/store/schema";

function createMemoryAdapter<T extends Record<string, unknown>>(
  schema: Schema<T>,
  options: { id: string },
) {
  let state = getSchemaDefaults(schema.definition);
  ...
  return {
    ...
    reset: () => {
      state = getSchemaDefaults(schema.definition);
      listeners.forEach((l) => l());
    },
...
We provide two official adapters:

1. **nuqs Adapter** (`@/lib/store/adapters/nuqs`) - URL-based state with SSR support
2. **Zustand Adapter** (`@/lib/store/adapters/zustand`) - Client-side state that integrates with existing Zustand stores

### React Compiler Compatibility

If you're using the React Compiler (`reactCompiler: true` in Next.js), be aware that TanStack Table's `table.getRowModel()` may be incorrectly memoized. Store the result in a variable before using it in JSX:

```tsx
// ✅ Correct - prevents React Compiler memoization issues
