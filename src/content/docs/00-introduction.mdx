...
This guide covers the data-table system end to end — from defining a table schema to rendering filters, columns, and row details. For a complete working example, see the [Full Example](/docs/full-example).
...
For server-side data, a **[Database](/docs/database)** adapter (Drizzle ORM) provides helper functions to build `WHERE` conditions, sorting, cursor pagination, and faceted counts. The **[Data Fetching](/docs/data-fetching)** section covers the API response shape and React Query integration.
