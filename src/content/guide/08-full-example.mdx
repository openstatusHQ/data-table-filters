...
Use `createTableSchema` to declare every column — display, filters, sorting, and row details — in one place. See [Table Schema](/guides/table-schema) for the full API.
...
Bridge the table schema to filter state with `generateFilterSchema`, then add non-column fields like sort and pagination. See [State Management](/guides/state-management) for details.
...
Create an API handler with `createDrizzleHandler` that implements three-pass filtering, faceted search, and cursor pagination. See [Database](/guides/database) and [Data Fetching](/guides/data-fetching) for the underlying concepts.
...
Wire up `useInfiniteQuery` with `DataTableInfinite` and populate filter fields dynamically from server-side facet data. See [Components](/guides/components) for all available props.
