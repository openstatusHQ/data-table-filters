...
Bidirectional sync between BYOS filter state and React Table's `columnFilters`. Needed when the table uses client-side filtering — keeps React Table's internal column filters in sync with the adapter state and vice versa. Not needed if filters only affect server-side queries.
