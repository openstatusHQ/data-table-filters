...
A step-by-step guide to wiring up the data table with [Drizzle ORM](https://orm.drizzle.team) and PostgreSQL. See the [live example](/drizzle) and the [Database](/guides/data-layer) page for the concepts behind the implementation.
...
The filter type you choose here (`checkbox`, `slider`, `input`, `timerange`) determines how the [three-pass filtering strategy](/guides/data-layer#three-pass-filtering) handles each column.
...
The API route uses `createDrizzleHandler` which implements the [three-pass filtering strategy](/guides/data-layer#three-pass-filtering), faceted search, counts, and cursor pagination:
