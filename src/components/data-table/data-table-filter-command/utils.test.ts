...
describe("getWordByCaretPosition", () => {
  it("extracts word at start of string", () => {
    expect(getWordByCaretPosition({ value: "hello world", caretPosition: 3 })).toBe("hello");
  });

  it("extracts word at end of string", () => {
    expect(getWordByCaretPosition({ value: "hello world", caretPosition: 8 })).toBe("world");
  });

  it("extracts middle word", () => {
    expect(getWordByCaretPosition({ value: "a bb ccc", caretPosition: 3 })).toBe("bb");
  });

  it("handles caret at start", () => {
    expect(getWordByCaretPosition({ value: "word", caretPosition: 0 })).toBe("word");
  });

  it("handles caret at end", () => {
    expect(getWordByCaretPosition({ value: "word", caretPosition: 4 })).toBe("word");
  });

  it("handles single word", () => {
    expect(getWordByCaretPosition({ value: "level:error", caretPosition: 5 })).toBe("level:error");
  });
});
...
describe("getFieldValueByType", () => {
  ...
  it("joins slider array values with dash delimiter", () => {
    const field = { type: "slider", label: "Latency", value: "latency", min: 0, max: 100 } as DataTableFilterField<any>;
    expect(getFieldValueByType({ field, value: [0, 100] })).toBe("0-100");
  });

  it("joins checkbox array values with comma delimiter", () => {
    const field = { type: "checkbox", label: "Level", value: "level" } as DataTableFilterField<any>;
    expect(getFieldValueByType({ field, value: ["error", "warn"] })).toBe("error,warn");
  });

  it("splits checkbox string value into array", () => {
    const field = { type: "checkbox", label: "Level", value: "level" } as DataTableFilterField<any>;
    expect(getFieldValueByType({ field, value: "error,warn" })).toEqual(["error", "warn"]);
  });

  it("joins timerange date array with dash delimiter", () => {
    const d1 = new Date("2024-01-01");
    const d2 = new Date("2024-01-31");
    const field = { type: "timerange", label: "Date", value: "date" } as DataTableFilterField<any>;
  ...
  it("returns input value as-is", () => {
    const field = { type: "input", label: "Path", value: "path" } as DataTableFilterField<any>;
...
describe("getFilterValue", () => {
  it("returns 1 when value includes currentWord (case-insensitive)", () => {
    expect(getFilterValue({ value: "level:error", search: "", currentWord: "level" })).toBe(1);
  });

  it("returns 0 when value does not match", () => {
    expect(getFilterValue({ value: "level:error", search: "", currentWord: "status" })).toBe(0);
  });

  it("handles suggestion: prefix", () => {
    expect(getFilterValue({ value: "suggestion:level:error", search: "error", currentWord: "" })).toBe(1);
    expect(getFilterValue({ value: "suggestion:level:error", search: "warn", currentWord: "" })).toBe(0);
  });

  it("handles checkbox comma-delimited query", () => {
    // "ams" is the last element being typed, so it should match
    expect(getFilterValue({ value: "regions:ams", search: "", currentWord: "regions:gru,ams" })).toBe(1);
  });
  ...
  it("returns 0 for already-selected checkbox value", () => {
    ...
    expect(getFilterValue({ value: "regions:ams", search: "", currentWord: "regions:ams,gru" })).toBe(0);
  });
});
